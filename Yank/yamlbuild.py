...
class YamlParser:
    """Base class to implement syntax validation for YAML scripts.

    Properties
    ----------
    options : dict
        The validated options for the experiment.
    yank_options : dict
        The options specified in the parsed YAML file that will be passed to Yank.
        This is a subset of the YamlParser.options specified in the script since
        some of them are used to configure the YamlParser object and not Yank.
    ...
    def _get_setup_dir(options):
        ...
        """
        return os.path.join(options['output_dir'], options['setup_dir'])

    @staticmethod
    def _get_experiment_dir(options):
        ...
        """
        return os.path.join(options['output_dir'], options['experiments_dir'])
    ...
    def _validate_experiment(self, experiment):
        """Check that the experiment syntax is correct.

        This also checks whether the components of the experiment can be found
        in the database, so molecules and solvents must be loaded before calling
        this. Experiment options are validated as well.

        Parameters
        ----------
        experiment : dict
            The YAML description for the experiment.
        ...
        if 'components' not in experiment:
            raise YamlParseError('Cannot find components in experiment')
        components = experiment['components']

        # Validate and check for unknowns
        try:
            utils.validate_parameters(experiment, experiment_template, check_unknown=True)
            utils.validate_parameters(components, components_template, check_unknown=True)
            self._validate_options(experiment.get('options', {}))
        ...
        if components['receptor'] not in self._db.molecules:
            err_msg = 'Cannot identify receptor in experiment'
        elif components['ligand'] not in self._db.molecules:
            err_msg = 'Cannot identify ligand in experiment'
        elif components['solvent'] not in self._db.solvents:
            err_msg = 'Cannot identify solvent in experiment'
    ...
    def _parse_experiments(self, yaml_content):
        """Load and validate experiment syntax.
        ...
        See Also
        --------
        _validate_experiment : called to validate the experiment syntax

        """
        self._experiments = yaml_content.get('experiments', {})
        self._validate_experiment(self._experiments)

class YamlExperiment(YamlParser):
    """Parse a YAML script describing a single experiment and run it.

    This does not support combinatorial experiments. Use YamlBuilder for that.

    """
    pass

class YamlBuilder(YamlParser):
    ...
    @staticmethod
    def _get_experiment_dir(options, experiment_subdir):
        """Return the path to the directory where the experiment output files
        should be stored.
        ...
        """
        experiment_dir = YamlParser._get_experiment_dir(options)
        return os.path.join(experiment_dir, experiment_subdir)
    ...
    def _parse_experiments(self, yaml_content):
        ...
        for exp_name, exp in self._expand_experiments():
            ...
            try:
                self._validate_experiment(exp)
            except YamlParseError as e:
                raise YamlParseError(' '.join((str(e), exp_name)))
